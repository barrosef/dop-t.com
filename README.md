...
| The decisions, told as they were made | `content/{en,pt-br}/decisions/` — one article per ADR, five parts each; see the spec's §11 |
| Search titles and descriptions, per page | `seo_title` and `description` in each page's front matter |
| Social images, favicons | `static/` — regenerated by hand from the mark and the two faces (not by the build) |
